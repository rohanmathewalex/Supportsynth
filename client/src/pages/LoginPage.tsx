...
const LoginPage: React.FC = () => {
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post('https://supportsynth.onrender.com/api/auth/login', {
        username, password
      });
  ...
  return (
    <Box sx={{ maxWidth: 300, mx: 'auto', mt: 5 }}>
      <Typography variant="h6" sx={{ mb: 2 }}>Login to SupportSynth</Typography>
      {error && <Alert severity="error">{error}</Alert>}
      <TextField label="Username" fullWidth margin="normal" value={username} onChange={e => setUsername(e.target.value)} />
      <TextField label="Password" type="password" fullWidth margin="normal" value={password} onChange={e => setPassword(e.target.value)} />
      <Button onClick={handleLogin} fullWidth variant="contained" sx={{ mt: 2 }}>Login</Button>
    </Box>
  );
};
